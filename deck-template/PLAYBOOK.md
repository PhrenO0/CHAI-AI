# 편집 가능한 한글 기획서 덱 만들기 — 실전 노하우

29장짜리 실제 프로젝트를 HTML과 PPTX 양쪽으로 만들면서 쌓인 것들이다.
이 폴더의 예시 덱(19장)은 여기 적힌 내용을 그대로 적용한 결과물이다.

핵심 전제는 하나다. **스크린샷을 붙이지 않는다.**
파워포인트에서 텍스트를 클릭해 고칠 수 있어야 PPT를 만든 의미가 있다.
...
index.html + styles/deck.css     ← 원본(source of truth). 브라우저에서 편집·검수
        ...
        │  같은 좌표계 · 같은 토큰
        ▼
build/build.js                   ← pptxgenjs 빌더. 네이티브 도형/텍스트로 재구성
        │
        ▼
build/embed_fonts.py             ← 한글 폰트를 파일에 임베드 (OOXML 직접 수정)
        │
        ▼
build/out/deck.pptx              ← 산출물
```
...
- 웹 배포와 PPTX를 한 소스에서 뽑는다
- 발표 직전에 문구를 고쳐야 할 때 브라우저에서 바로 된다

대가도 있다. **HTML과 빌더를 항상 두 번 고쳐야 한다.** 한쪽만 고치면 조용히 어긋난다.
실제 프로젝트에서 HTML은 27장, PPTX는 28장인 상태로 며칠 굴러간 적이 있다.
...
가장 중요한 결정. 파워포인트 와이드스크린은 **13.333 × 7.5 inch**이고,
이건 **1280 × 720 px @ 96dpi**와 정확히 같다.

```js
const IN = 1 / 96;
const px = (n) => n * IN;                  // px → inch
const SLIDE_W = 13.333, SLIDE_H = 7.5;
const PAD = px(72);                        // 0.75 = CSS의 --pad
...
CSS 변수와 JS 상수를 같은 값으로 유지한다. (`styles/deck.css` ↔ `build/build.js`의 `C`)
...
**강조색을 하나로 제한하는 게 제일 효과가 컸다.** 파란색 하나만 쓰면 어디가 중요한지 저절로 드러난다.
브랜드 색을 바꿀 때는 `--blue` 계열 4개와 `C.blue` 계열 4개만 교체하면 된다.

---

## 4. 헬퍼 함수로 컴포넌트화

매 장을 개별로 짜면 20장에서 무너진다. 반복 요소를 함수로 뽑는다.
(전문은 `build/build.js` 상단)

```js
slide(color)                          // 배경색 지정된 새 슬라이드
T(s, str, o)                          // 텍스트 (fontFace·margin 기본값 주입)
SH(s, type, x, y, w, h, o)            // 도형
card(s, x, y, w, h, o)                // 라운드 카드
photo(s, x, y, w, h, file, o)         // 사진 (cover 크롭 + 그림자)
imgFrame(s, x, y, w, h, label, o)     // 이미지 플레이스홀더
pill(s, x, y, w, h, str, o)           // 알약 버튼
pillRow(s, x, y, items, o)            // 알약 자동 줄바꿈 배치
bullet(s, x, y, w, str, o)            // 불릿 (원 + 텍스트)
marker(s, label, y, dark)             // 눈썹 라벨 (파란 짧은 선 + 텍스트)
heading(s, x, y, w, h, lines, o)      // 강조색 섞인 여러 줄 제목
handoff(s, y, str)                    // 다음 장으로 넘기는 한 줄
vlabel(s, str, cx, cy, len, o)        // 세로 축 라벨
foot(s, no, dark)                     // 러닝 푸터 + 페이지 번호
divider(en, num, title, items)        // 섹션 디바이더
```
...
```js
// ❌ 깨짐
T(s, [{ text: "오늘의 공기를,\n오늘 " }, { text: "바꾼다", options: { color: C.blue } }]);

// ✅ breakLine 사용
heading(s, PAD, 1.1, CW, 1.0,
  [[{ t: "오늘의 공기를," }],
   [{ t: "오늘 " }, { t: "바꾼다", hl: true }, { t: "." }]], { size: 46 });
```

### 5-2. `breakLine`은 반드시 `options` **안**에 넣는다

이 템플릿을 만들면서도 네 곳에서 틀렸다. 밖에 두면 **에러 없이 조용히 무시된다.**
그래서 "왜 줄바꿈이 안 되지"를 렌더해 보고서야 알게 된다.

```js
// ❌ 무시된다
{ text: "첫 줄", options: { color: C.body }, breakLine: true }
...
```bash
grep -n 'options: {[^}]*} *, *breakLine' build/build.js
```

### 5-3. `fill: "transparent"`는 파일을 깨뜨린다
...
반투명이 필요하면 `transparency`(0~100)를 쓴다. CSS의 `backdrop-filter`는 대응물이 없어서,
회색 리빌 같은 효과는 **흰 반투명 사각형을 위에 덮는 방식**으로 만든다.

```js
SH(s, "rect", x, y, w, h, { fill: { color: "F5F5F7", transparency: 22 }, line: { type: "none" } });
```
...
### 5-5. 누적 막대는 아래에서 위로 역산한다

세그먼트 높이를 먼저 구하고 `y`를 빼 나가야 음수 높이가 안 생긴다.

```js
let y = baseY;
segs.forEach(([ratio, color]) => {
  const h = total * ratio;
  y -= h;                                   // 먼저 올린 뒤 그린다
  SH(s, "rect", cx - barW / 2, y, barW, h, { fill: { color }, line: { type: "none" } });
});
```

### 5-6. 스크립트로 JS를 생성할 때

Python `re.sub()`의 치환 문자열에서 `\\n`은 **실제 개행으로 해석된다.** JS 리터럴이 깨진다.
`re.sub` 대신 `str.replace`를 쓰거나, 치환 후 반드시 문법 검사를 한다.

---

## 6. 한글 폰트 임베딩

한글 PPT의 가장 큰 함정. 웹폰트는 받는 사람 PC에 없다.
맑은 고딕으로 폴백되면 자간·굵기가 무너져 디자인이 전부 깨진다.

두 단계다. 스크립트는 `build/otf2ttf.py` → `build/embed_fonts.py`.
...
maxp.maxZones = 1        # CFF의 maxp에는 TT 전용 필드가 없다. 안 채우면 KeyError: 'maxZones'
...
Regular와 Bold 두 개만 넣으면 충분하다(각 약 3MB, 최종 파일 +2.5MB 정도).

### 6-2. OOXML 직접 수정

pptxgenjs에 폰트 임베딩 기능이 없어서 zip을 열어 손으로 넣는다. 네 곳이다.
...
## 7. QA 파이프라인 — 이게 없으면 20장을 못 만든다

### 7-1. 넘침 자동 검출 (`build/qa/fit.js`)

가장 크게 시간을 아껴준 도구. **푸터를 침범했는지**를 기계가 판정한다.

```js
const rows = await page.$$eval('.slide', els => els.map(e => {
  const inner = e.querySelector('.slide__inner');
  const foot  = e.querySelector('.slide__foot');
  if (!inner || !foot) return null;                 // 표지·디바이더는 건너뛴다
...
# checked  : 14 slides
# OVERLAP  : none      ← 이게 none이어야 통과
# TIGHT    : none      ← gap 10px 미만. PPTX에서 겹칠 가능성이 높다
```

`scrollHeight > clientHeight`만 보면 안 된다. 절대배치 요소(오버레이 등) 때문에 오탐이 난다.
**마지막 자식의 바닥 vs 푸터의 top**을 비교하는 게 정확했다.

측정 전에 `zoom`을 1로 고정하고 툴바를 숨겨야 한다. 안 하면 값이 스케일된다.

```js
await page.addStyleTag({ content: '.deck{zoom:1 !important}.toolbar{display:none !important}' });
```

### 7-2. 어디가 넘치는지 측정 (`build/qa/meas.js`)

```bash
$ NODE_PATH=/opt/node22/lib/node_modules node build/qa/meas.js s16
{ "pad": "72px", "innerH": 720, "usable": 576, "sum": 620, "footGap": -8,
  "kids": [ {"cls":"slide-head","h":131}, {"cls":"row","h":404},
            {"cls":"card","h":74,"mt":"11px"} ] }
```

`131 + 404 + 11 + 74 = 620` vs 가용 `576` → **44px 초과**. 어디를 깎을지 바로 보인다.

### 7-3. 스크린샷 (`build/qa/shot.js`)

```bash
NODE_PATH=/opt/node22/lib/node_modules node build/qa/shot.js        # 전 장
NODE_PATH=/opt/node22/lib/node_modules node build/qa/shot.js s16    # 한 장
```

### 7-4. PPTX 렌더 검수 (`build/qa/render.sh`)

```bash
apt-get install -y libreoffice-impress poppler-utils   # impress 없으면 무조건 실패한다

bash build/qa/render.sh build/out/deck.pptx            # 전체
bash build/qa/render.sh build/out/deck.pptx 16 16      # 16장만
```

함정 두 개로 한참 헤맸다.

- **`libreoffice` 코어만 깔려 있으면** impress 필터가 없어서 전부 "source file could not be loaded"로 실패한다
- **한글 파일명을 그대로 주면** 같은 에러가 뜬다. ASCII 이름으로 복사해서 변환한다
- 프로필이 꼬이면 `rm -rf ~/.config/libreoffice`

### 7-5. 컨택트 시트 (`build/qa/sheet.py`)

20장을 한 장씩 열어보면 지친다. 3×3으로 붙여 한 번에 훑는다.

```bash
python3 build/qa/sheet.py build/qa/pptx 3 3      # → sheet-1.png, sheet-2.png …
SCALE=0.6 python3 build/qa/sheet.py build/qa/out 2 2
```

### 7-6. 파일 유효성 검사

```bash
python3 <pptx skill>/scripts/office/validate.py build/out/deck.pptx
...
PPTX는 좌표가 고정이라 `margin-top:auto` 같은 트릭이 없다.
...
### ✅ 협업자가 이미 만든 파일에서 추출 — 의외로 최고의 소스

검색으로 못 찾던 과거 광고 이미지를 여기서 다 찾았다. **검색보다 빠르고 정확하다.**

```bash
unzip -q 기존파일.pptx -d x && ls x/ppt/media/
```
...
### ✅ YouTube 썸네일 — 광고 스틸이 필요할 때

원본 1280×720을 그대로 준다. 슬라이드 프레임과 비율이 같아서 크롭이 거의 필요 없다.
...
### ✅ Wikipedia pageimages API — 유명 인물·마스코트
...
있는 항목과 없는 항목이 갈린다. 미리 확인하고 대체안을 준비한다.
...
`gsrnamespace=6`으로 파일만 걸러도 PDF 스캔본만 올라온다. 위의 페이지 이미지 API가 낫다.
...
한 번은 16:9 인물 이미지를 `550×128px`(4.3:1) 프레임에 넣어서 피사체가 완전히 사라졌다.
"이미지가 깨졌나?" 하고 원본을 뒤졌지만 원본은 정상이었다. **프레임이 문제였다.**

| 상황 | 처리 |
|---|---|
| 프레임 비율 ≈ 원본 비율 | `object-fit: cover` / `sizing: cover` |
...
# 작은 원본은 프레임보다 크게 올려둔다. 인쇄 시 흐려지지 않는다
if im.width < 900:
...
축 두 개를 동시에 조합해야 하니 청중이 "가로가 인지도고 세로가 효과니까, 여긴 인지 높고 효과 낮은 자리네"를
머릿속에서 계산해야 한다. 실제로 "이해가 어렵다"는 피드백을 받았다.

**한 방향으로 읽히는 단계형으로 바꾸니 해결됐다.** (`.ladder` 컴포넌트, 예시 15장)

```
적극 ─ 기존 자산 전면 복원  [유력]   기억 O · 적합 O
기본 ─ 새 자산으로 대체              기억 X · 적합 O
제한 ─ 아카이브 용도로만             기억 O · 적합 X
중단 ─ 전략에서 제외                 기억 X · 적합 X
```

조건을 `O/X` 태그로 각 줄에 붙이면 축 해석이 필요 없어진다.
그리고 고를 줄만 파란색으로 두니 "여기까지가 간다는 쪽"이 저절로 구분됐다.

세그먼트 지도처럼 "어디를 고를지"를 보여줄 때만 사분면을 쓴다(예시 09장).
**의사결정에는 쓰지 않는다.**
...
비용이 거의 안 드는데 효과가 컸다. (`.handoff` / `handoff()`)

```
06장 끝 → "그렇다면 새로 만들기 전에, 이미 가진 것을 먼저 본다."
12장 끝 → "그렇다면 제품은 무슨 역할을 맡아야 하는가."
14장 끝 → "이 역할을 전달할 자산을 어디까지 쓸 것인가."
15장 끝 → "그 O/X는 무엇으로 판정하는가."
```

여기에 눈썹 라벨로 현재 위치를 표시한다: `03 · 브랜드 전략 · 02 자산 활용 수준`

### 숫자를 쓸 때는 척도를 설명한다

"2.8★" 같은 걸 그냥 놓으면 아무 의미가 없다. 실제로 "이게 뭘 의미하는지 알 수 없다"는 지적을 받았다.

고친 방법 (예시 16장, `.rate` + `.spec`):
...
```js
const w = (v) => (v - MIN) / (MAX - MIN);
```

### 한 장에 프레임 하나

한 장에 이론 프레임 3개를 얹었더니 "정보가 너무 많다"는 지적을 받았다.
측정 프레임은 **측정을 다루는 장으로 옮겼더니** 양쪽이 다 좋아졌다.
...
전 장을 "카드 3개"로 만들면 티가 난다. 이 템플릿에 컴포넌트를 30개 넘게 넣어둔 이유다.
같은 정보라도 타임라인·사다리·퍼널·누적막대 중 어느 걸로 보여주는지가 완성도를 가른다.

---

## 12. 카피 — AI 티 걷어내기

`COPY-GUIDE.md`에 자세히 정리했다. 핵심만:
...
초안으로 채운 문구에는 눈에 보이는 태그(`.draft` = "초안")를 달아 교체 대상임을 표시한다.
...
| 추정치는 실측치로 바꾼다 | 소모품 단가 추정 720원 → 실측 **544원**. 손익분기가 42 → 37로 바뀜 |
| 글로벌 수치를 국내 수치처럼 쓰지 않는다 | 컨설팅사 70%는 13개국 값. `해외 기준 · 참고` 띠로 분리 |
| 없는 통계는 없다고 쓴다 | "국내 공식 통계는 확보 전이며, 보유율·성장률로 대신했다" |
| 가정치는 가정치로 표기 | 단가·원가 추정값 |

마지막 항목이 제일 중요하다. **모르는 걸 아는 척하면 질의응답에서 무너진다.**
"확보 중"이라고 쓰면 오히려 신뢰가 올라간다.

이 템플릿에는 `.tbd` (출처 확인 예정) 와 `.draft` (초안) 두 개의 표시 태그가 있다.
값을 비워두는 대신 **왜 비었는지**를 슬라이드에 남기는 용도다.
...
- 대비: 치환 후 `grep -o 'id="s[0-9b]*"' index.html | sort | uniq -d`
- 번호 표기는 처음부터 통일한다
...
### CSS Grid의 `grid-row: 1 / -1`이 1행만 차지했다

중앙 허브 표(`.ptable`)에서 허브가 전 행을 가로질러야 하는데,
`-1`은 **명시적** 그리드의 마지막 선을 가리킨다. 행을 선언하지 않으면 1행만 차지하고
2행 이후 셀들이 가운데 열로 흘러들어가 표가 무너진다.

```css
.ptable { grid-template-rows: repeat(var(--rows, 4), auto); }   /* 행 수를 명시 */
```
```html
<div class="ptable" style="--rows:4">
```
...
NODE_PATH=/opt/node22/lib/node_modules node build/qa/meas.js s16
# 4. 눈으로 확인
NODE_PATH=/opt/node22/lib/node_modules node build/qa/shot.js s16
# 5. build.js에 같은 변경 반영  ← 이걸 빼먹으면 두 파일이 어긋난다
# 6. 빌드 → 폰트 임베딩 → 검증
NODE_PATH=/opt/node22/lib/node_modules node build/build.js
python3 build/embed_fonts.py build/out/deck.pptx --regular …-Regular.ttf --bold …-Bold.ttf
python3 <pptx skill>/scripts/office/validate.py build/out/deck.pptx
# 7. PPTX 렌더해서 그 장만 확인 (HTML보다 빡빡하므로 필수)
bash build/qa/render.sh build/out/deck.pptx 16 16
# 8. 커밋
```
